    ...
    OneByOne([
      ...
      , function (cb) {
            ...
            Glob(basePath, function (err, libFiles) {
                ...
                package.filename = files[0].replace(Path.join(self.repo.cwd, package.npmFileMap[0].basePath) + "/", "");
                cb();
            });
        }
      ...
      , self.git.exec.bind(self.git, "pull origin master")
        // Create the new branch
      , function (cb) {
            Logger.log("Creating a new branch for your library.", "info");
            self.git.exec(OArgv({
                _: "importer-" + package.name
              , b: true
            }, "checkout"), cb)
        }
        // Add files
